...
private:
  ...
  void generate_point_based_features ()
  {
    ...
    generate_multiscale_feature_variant_4<Eigenvalue> ();
#else
  ...
  template <typename ColorMap>
  struct Feature_adder_color_channel : public Feature_adder
  {
    typedef Classification::Feature::Color_channel<GeomTraits, PointRange, ColorMap> Color_channel;
    ...
    Feature_adder_color_channel (Point_set_feature_generator* generator, ColorMap color_map, std::size_t scale,
                                 std::size_t channel)
      : Feature_adder (generator, scale), color_map (color_map),
        channel (channel) { }
    ...
    void operator() () const
    {
      Feature_handle fh = generator->m_features->template add<Color_channel> (generator->m_input, color_map,
                                                                              (typename Color_channel::Channel)(channel));
      std::ostringstream oss;
      oss << fh->name() << "_" << scale;
      fh->set_name (oss.str());
    }
  ...
  void generate_color_based_features(ColorMap color_map)
  {
    ...
    launch_feature_computation (new Feature_adder_color_channel<ColorMap> (this, color_map, 0, 0));
    launch_feature_computation (new Feature_adder_color_channel<ColorMap> (this, color_map, 0, 1));
    launch_feature_computation (new Feature_adder_color_channel<ColorMap> (this, color_map, 0, 2));
#else
...
#ifdef DO_NOT_USE_EIGEN_FEATURES
  template <typename Feature_type>
  struct Feature_adder_variant_4 : public Feature_adder
  {
    using Feature_adder::generator;
    using Feature_adder::scale;
    std::size_t idx;

    Feature_adder_variant_4 (Point_set_feature_generator* generator, std::size_t scale, std::size_t idx)
      : Feature_adder (generator, scale), idx (idx) { }
  ...
  template <typename Feature_type>
  void generate_multiscale_feature_variant_4 ()
  {
    for (std::size_t i = 0; i < m_scales.size(); ++ i)
    {
      launch_feature_computation (new Feature_adder_variant_4<Feature_type> (this, i, 0));
      launch_feature_computation (new Feature_adder_variant_4<Feature_type> (this, i, 1));
      launch_feature_computation (new Feature_adder_variant_4<Feature_type> (this, i, 2));
    }
  }
#endif
